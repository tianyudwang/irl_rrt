...
def visualize_env(env, controls):
...
def makeStartState(
    ...
    for i in range(len(pos)):
        ...
        start[i] = pos[i]
    assert err == 0, "Start state is out of bounds"
...
class AntStatePropagator(oc.StatePropagator):
    def __init__(
        ...
        self.qpos_temp = np.zeros_like(self.agent_model.sim.data.qpos)
        self.qvel_temp = np.zeros_like(self.agent_model.sim.data.qvel)
        self.action_temp = np.zeros_like(self.agent_model.sim.data.ctrl)
