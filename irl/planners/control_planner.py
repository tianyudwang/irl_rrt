...
class Maze2DControlPlanner(Maze2DBasePlanner):
    ...
    def plan(
        ...
        total_solveTime: Optional[float] = 30.0
...
class Maze2DKPIECEPlanner(Maze2DControlPlanner):
    def __init__(self, env):
        super().__init__(env)

        self.planner = oc.KPIECE1(self.si)

        projection = self.get_ProjectionEvaluator()
        self.space.registerProjection("2DProjection", projection)
        self.planner.setProjectionEvaluator("2DProjection")
        # planner.setProjectionEvaluator(projection)

        self.ss.setPlanner(self.planner)

    def get_ProjectionEvaluator(self):
        projection = MazeProjection(self.space)
        # projection.setCellSizes(list2vec([1., 1.]))
        # import ipdb; ipdb.set_trace()
        return projection

class MazeProjection(ob.ProjectionEvaluator):
    """Projection of the 4D state space to 2D in xy"""
...
class AntMazeKPIECEPlanner(AntMazeControlPlanner):
    def __init__(self, env):
        ...
        projection = self.get_ProjectionEvaluator()
        self.space.registerProjection("2DProjection", projection)
        planner.setProjectionEvaluator("2DProjection")
        # self.planner.setProjectionEvaluator(projection)
